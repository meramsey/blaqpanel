...
## Objective Fix session issues on blaqpanel and standardized session paths.
...
command="/usr/local/blaqpanel/bin/cleansessions >/dev/null 2>&1"
job="09,39 * * * * $command"
cat <(grep -i -v "$command" <(crontab -l)) <(echo "$job") | crontab -

echo "Checking cleansessions file"
cat /usr/local/blaqpanel/bin/cleansessions

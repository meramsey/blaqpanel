...
# @Version: 2.0
# Source: https://github.com/litespeedtech/ls-cloud-image/blob/master/Setup/ols2ent-v2.sh
# Last modified : 2021-09-26
# Reason: Remove echo ansi colors so console in panel works rendering wise
# *********************************************************************/
...
webadmin_reset() {
    ...
    echo "WebAdmin Console password has been set to: ${WEBADMIN_PASS}"
}
...
check_no_panel(){
    ...
    if [ ! -z "${PANEL}" ]; then
        echo "Detect control panel: ${PANEL}, exit!"; exit 1 
    fi 
}
...
check_pkg_manage(){
    if hash apt > /dev/null 2>&1 ; then
      ...
      echo 'can not detect package management tool ...'
      exit 1
    fi
}
...
check_php(){
    ...
    if [ ${?} = 0 ]; then
        ...
        if [ ${?} = 0 ]; then
            echo 'PHP path update' 
            PHP=$(find ${LS_DIR}/lsphp* -path \*bin/php | head -n 1)
        fi
        which ${PHP} >/dev/null
        if [ ${?} != 0 ]; then
            echo 'PHP path does not exist, exit!'; exit 1
        fi
    fi
}
...
restore_ols() {
    if ${LS_DIR}/bin/lshttpd -v | grep -q Open ; then
        echo 'You already have OpenLiteSpeed installed...'
        exit 0
    fi
    ...
    if [[ ! -d ${STORE_DIR}/${ols_backup_dir} ]] ; then
      echo 'The dir seems not exists.'
      exit 1
    fi
    if [[ ! -f ${STORE_DIR}/${ols_backup_dir}/conf/httpd_config.conf ]] ; then
        echo 'Main conf file is missing...'
        exit 1
    else
        ...
        echo 'OpenLiteSpeed Restored...'
        webadmin_reset
    fi
}

download_lsws() {
    echo 'Download LiteSpeed Web Server.'
...
write_license() {
    if [[ ${LICENSE_KEY} == 'TRIAL' ]] ; then
        ...
        if ./lshttpd -V |& grep  "ERROR" ; then
            ./lshttpd -V
            echo 'It apeears to have some issue with license , please check above result...'
            exit 1
        fi
        echo 'License seems valid...'
    else
...
check_license(){
    ...
    if [[ ${TMP} == "-" ]] && [[ ${TMP2} == "-" ]] && [[ ${TMP3} == "-" ]] && [[ ${KEY_SIZE} == "19" ]] ; then
        echo 'License key format check...'
    elif [[ ${1} == "trial" ]] || [[ ${1} == "TRIAL" ]] || [[ ${1} == "Trial" ]] ; then
        echo 'Trial license format check...'
        LICENSE_KEY='TRIAL'
    else
        echo 'License key seems incorrect, please verify'
        exit 1
    fi
}

license_input() {
    echo -e "\nPlease note that your server has $TOTAL_RAM MB RAM"
	
	if [ "$TOTAL_RAM" -gt 2048 ]; then
        echo "$TOTAL_RAM is greater than 2048 MB RAM"
		echo -e "If you are using Free Starter LiteSpeed license, It will not start due to 2GB RAM limit."
	fi
    
    echo -e "If you do not have any license, you can also use trial license (if server has not used trial license before), type TRIAL\n"
    while true; do
        ...
        if [ -z "${LICENSE_KEY}" ] ; then
            ...
            echo -e "The serial number you input is: ${LICENSE_KEY}"
...
uninstall_ols() {
    echo 'Uninstall OpenLiteSpeed.'
    if [[ -f ${LS_DIR}/conf/httpd_config.conf ]] ; then
        DATE=`date +%Y-%m-%d_%H%M`
        mkdir ${STORE_DIR}/OLS_backup_$DATE/
        echo "Backing up current OpenLiteSpeed configuration file to ${STORE_DIR}/OLS_backup_${DATE}/"
        ...
        check_return
        echo 'OpenLiteSpeed successfully removed...'
    fi
}
...
install_lsws() {
    echo 'Installing LiteSpeed Enterprise...'
    ...
    if [[ ${?} != "0" ]] ; then
        while [ ${COUNTER} -le 4 ]; do
            ...
            if [[ ${?} == "0" ]] ; then
                ...
                echo 'Unable to install LiteSpeed Enterprise, switching back to OpenLiteSpeed...'
                restore_ols
                exit 1
            fi
            COUNTER=$((var+1))
    ...
    echo 'LiteSpeed Enterprise installed...'
    set_ent_cache
    set_ent_htaccess
    rm_lsws_autoupdate
    restart_lsws
...
check_no_lsws(){
    if ${LS_DIR}/bin/lshttpd -v | grep -q Enterprise ; then
        echo 'You have already installed LiteSpeed Enterprise...'
        exit 1
    fi    
}

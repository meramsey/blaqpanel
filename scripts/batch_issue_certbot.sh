...
for dir in $(ls -d /var/www/*/html); do
	...
	certbot certonly --non-interactive --agree-tos -m admin@${domain} --webroot -w ${DOCHM} -d ${domain} -d www.${domain}||certbot certonly --non-interactive --agree-tos -m ${email} --webroot -w ${DOCHM} -d ${domain}
